import { CHANGE_LOCATION } from '@/Navigation/reducer'
import api from '../../api'
      ...
      fetch(api.getPromoCards(limit))
        ...
        .then((ads) => {
          const result = posts.map((post, index) => {
            ...
            return index > 0 && div === 0 && ad ? [post, ad] : post
          })
...
export const getPostsByCategory = (
  ...
  fetch(api.getPostsByCategory(category, limit, page))
    ...
    .then((result) => {
      if (mainCategory) {
        return dispatch(
          fetchSubCategorySuccess(
            result, category, result.length > 0 ? isRefresh : true, mainCategory,
          ),
        )
      }
      ...
      return dispatch(fetchFail([], category, false))
    })
...
export default function (state = initialState, action) {
  switch (action.type) {
    ...
    case FETCH_POSTS_SUCCESS: {
      ...
      return {
        ...state,
        data: [...oldPosts, ...action.payload.data],
        isLoading: false,
      }
    }
    case FETCH_POSTS_BY_SUBCATEGORY_SUCCESS: {
      const {
        category, mainCategory, data, isRefresh,
      } = action.payload
